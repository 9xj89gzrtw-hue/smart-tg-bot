...
### R33: Никогда не используй skip-if-active в GH Actions
- skip-if-active видит САМ СЕБЯ как active (total_count=1) → skip=true
- Бот НИКОГДА не запускается
- Решение: всегда запускать бот, убрать skip logic полностью
- Доказательство: 8 итераций дебага чтобы найти эту причину

### R34: Тестируй модель на ПРОСТОМ вопросе перед deployment
- DeepSeek-V4-Flash: выводит "We need to answer..." вместо ответа
- Qwen3.5-9B: возвращает пустой ответ
- Llama-3.3-70B: отвечает прямо "391" ✓
- Тест: "What is 2+2? Number only." → должен вернуть "4"
- Если модель не проходит → не использовать как PRIMARY

### R35: || true скрывает ошибки в GH Actions
- `timeout 280m node bot.mjs || true` → если бот падает, run = success
- Это СКРЫВАЕТ проблему — кажется что всё работает
- Решение: убери || true для основного бота, оставь только для cleanup steps
